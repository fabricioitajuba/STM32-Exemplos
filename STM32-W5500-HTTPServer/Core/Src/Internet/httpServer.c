...
#include "../../Inc/Internet/httpServer.h"
#include "../../Inc/Internet/httpParser.h"
#include "../../Inc/Internet/httpUtil.h"
...
static void http_process_handler(uint8_t s, st_http_request * p_http_request)
{
	...
	switch (p_http_request->METHOD)
	{
		...
		case METHOD_GET :
			...
			else if(!strcmp((char *)uri_name, "ledon.html")){
				HAL_GPIO_WritePin(GPIOC, GPIO_PIN_13, GPIO_PIN_RESET); //Adicionada
			}
			else if(!strcmp((char *)uri_name, "ledoff.html")){
				HAL_GPIO_WritePin(GPIOC, GPIO_PIN_13, GPIO_PIN_SET); //Adicionada
			}
