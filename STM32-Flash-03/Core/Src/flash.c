...
void SaveStructToFlash(MyData_t *data) {
    ...
    // Erase the page containing the data
    EraseInitStruct.TypeErase = FLASH_TYPEERASE_PAGES;
    EraseInitStruct.PageAddress = FLASH_STORAGE;
    ...
    if (status == HAL_OK) {
        // Program the struct data to Flash
        uint32_t *src_ptr = (uint32_t *)data;
        uint32_t dest_addr = FLASH_STORAGE;
        size_t size_in_words = sizeof(MyData_t) / sizeof(uint32_t);

        for (size_t i = 0; i < size_in_words; i++) {
            HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, dest_addr, src_ptr[i]);
...
void LoadStructFromFlash(MyData_t *data) {
    // Copy data from Flash to RAM-based struct
    memcpy(data, (void *)FLASH_STORAGE, sizeof(MyData_t));
}
